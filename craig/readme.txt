...
I've created a list of some [[https://github.com/craigfrancis/php-is-literal-rfc/blob/main/justification.md#common-mistakes|common escaping mistakes]].
...
Those that attempt to address injection vulnerabilities, do so via Taint Checking (flawed), and are [[https://github.com/vimeo/psalm/commit/2122e4a1756dac68a83ec3f5abfbc60331630781|often incomplete]].
...
Google uses a similar approach in Go to identify "compile time constants", which isn't as good as a run time solution (e.g. the //WHERE IN// issue), but it works, and is used by [[https://blogtitle.github.io/go-safe-html/|go-safe-html]] and [[https://github.com/google/go-safeweb/tree/master/safesql|go-safesql]].
...
Alternatives suggestions have included //is_from_literal()// from [[https://news-web.php.net/php.internals/109197|Jakob Givoni]], //is_safe_string()// which is just asking for trouble. Other terms have included "compile time constants" and "code string".
...
A [[https://github.com/craigfrancis/php-is-literal-rfc/blob/main/tests/001.phpt|basic string concat test]], just focusing on string concat (worst case scenario), shows a 1.3% increase in processing time (1.341s to 1.358s = +0.017s). And Máté Kocsis has run some tests with more typical projects:
...
As noted by [[https://news-web.php.net/php.internals/87667|Dennis Birkholz]], Systems/Frameworks that define certain variables (e.g. table name prefixes) without the use of a literal (e.g. ini/json/yaml files), might need to make some changes to use this (depending on where they use the is_literal check).
